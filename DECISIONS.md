...
12. **Judge design.** The judge must not be the reply writer (Qwen). Free-tier limits decided the rest: Gemini 2.5 Flash allows only 20 requests a day, and `gpt-oss-120b` used up its 200K daily tokens partway through the test split, so every split was re-graded with `gpt-oss-20b`, the final judge. The judge's only reference is the brand's actual reply: giving it retrieved replies would hand the retrieval baseline its own answer. Cases are graded in shuffled batches of 12 to stay within token limits.

13. **The judge is validated four ways, not one**: agreement with 60 blind human ratings (87% on acceptability, κ = 0.73); a planted-defect stress test (69 of 72 broken replies rejected); batched vs one-at-a-time grading (92% agreement on acceptability); and a second judge model on the same 150 dev replies (86% agreement, but a 14-point swing in the agent's acceptable rate).
